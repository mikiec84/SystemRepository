...
## BEHAVIOR FILES
shopt -u | grep -q nullglob && changed=true && shopt -s nullglob
for entry in "$REFERENCED_PROJECT_SmartGazeboBaseServer"/model/*.smartTcl
do
  DEPLOY_COMPONENT_BEHAVIOR_MODEL_FILES_SmartGazeboBaseServer="$DEPLOY_COMPONENT_TCL_MODEL_FILES_SmartGazeboBaseServer $entry"
done
[ $changed ] && shopt -u nullglob; unset changed

echo "$DEPLOY_COMPONENT_BEHAVIOR_MODEL_FILES_SmartGazeboBaseServer "
#########################
...
if [ ! "$DEPLOY_COMPONENT_BEHAVIOR_MODEL_FILES_SmartGazeboBaseServer" = "" ]; then
	cp -rv $DEPLOY_COMPONENT_BEHAVIOR_MODEL_FILES_SmartGazeboBaseServer $TMPDIR/behaviorFiles/ 2>&1
fi
