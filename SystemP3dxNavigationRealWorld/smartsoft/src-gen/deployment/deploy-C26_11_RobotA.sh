...
## BEHAVIOR FILES
shopt -u | grep -q nullglob && changed=true && shopt -s nullglob
for entry in "$REFERENCED_PROJECT_SmartLaserLMS200Server"/model/*.smartTcl
do
  DEPLOY_COMPONENT_BEHAVIOR_MODEL_FILES_SmartLaserLMS200Server="$DEPLOY_COMPONENT_TCL_MODEL_FILES_SmartLaserLMS200Server $entry"
done
[ $changed ] && shopt -u nullglob; unset changed

echo "$DEPLOY_COMPONENT_BEHAVIOR_MODEL_FILES_SmartLaserLMS200Server "
#########################
...
## BEHAVIOR FILES
shopt -u | grep -q nullglob && changed=true && shopt -s nullglob
for entry in "$REFERENCED_PROJECT_SmartPioneerBaseServer"/model/*.smartTcl
do
  DEPLOY_COMPONENT_BEHAVIOR_MODEL_FILES_SmartPioneerBaseServer="$DEPLOY_COMPONENT_TCL_MODEL_FILES_SmartPioneerBaseServer $entry"
done
[ $changed ] && shopt -u nullglob; unset changed

echo "$DEPLOY_COMPONENT_BEHAVIOR_MODEL_FILES_SmartPioneerBaseServer "
#########################
...
src-gen/combined-ini-files/SmartCdlServer.ini
...
src-gen/combined-ini-files/SmartPlannerBreadthFirstSearch.ini
$SMART_ROOT_ACE/lib/libCommBasicObjects.so*
$SMART_ROOT_ACE/lib/libCommNavigationObjects.so*
...
if [ ! "$DEPLOY_COMPONENT_BEHAVIOR_MODEL_FILES_SmartLaserLMS200Server" = "" ]; then
	cp -rv $DEPLOY_COMPONENT_BEHAVIOR_MODEL_FILES_SmartLaserLMS200Server $TMPDIR/behaviorFiles/ 2>&1
fi
...
if [ ! "$DEPLOY_COMPONENT_BEHAVIOR_MODEL_FILES_SmartPioneerBaseServer" = "" ]; then
	cp -rv $DEPLOY_COMPONENT_BEHAVIOR_MODEL_FILES_SmartPioneerBaseServer $TMPDIR/behaviorFiles/ 2>&1
fi
