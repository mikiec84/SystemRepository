...
DEPLOYMENT_DIRECTORY=SystemTrafficLight.deployment/
...
source src/predeploy-ComponentTrafficLightTest.sh 2>&1
...
DEPLOY_COMPONENT_FILES_PATHS_ComponentTrafficLightTest=""
for I in $DEPLOY_COMPONENT_FILES; do
	if ls $REFERENCED_PROJECT_ComponentTrafficLightTest/$I > /dev/null 2>&1; then
		DEPLOY_COMPONENT_FILES_PATHS_ComponentTrafficLightTest="$DEPLOY_COMPONENT_FILES_PATHS_ComponentTrafficLightTest $REFERENCED_PROJECT_ComponentTrafficLightTest/$I"
	elif ls $I > /dev/null 2>&1; then
		DEPLOY_COMPONENT_FILES_PATHS_ComponentTrafficLightTest="$DEPLOY_COMPONENT_FILES_PATHS_ComponentTrafficLightTest $I"
	fi
done
...
src/ComponentTrafficLightTest_data
src/startstop-hooks-ComponentTrafficLightTest.sh
$SMART_ROOT_ACE/bin/ComponentTrafficLightTest
src-gen/combined-ini-files/ComponentTrafficLightTest.ini
$SMART_ROOT_ACE/lib/libDomainHMI.so*
...
#rsync -l -r -v --progress --exclude ".svn" $DEPLOY_COMPONENT_FILES_PATHS_ComponentTrafficLightTest $TMPDIR/ComponentTrafficLightTest_data/
if [ ! "$DEPLOY_COMPONENT_FILES_PATHS_ComponentTrafficLightTest" = "" ]; then
