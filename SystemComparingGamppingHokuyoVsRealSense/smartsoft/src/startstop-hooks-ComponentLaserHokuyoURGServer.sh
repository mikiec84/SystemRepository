#!/bin/bash
#
# Start-Stop-Hooks for component ComponentLaserHokuyoURGServer
#
...
pre-start)
	...
	bash startstop-hooks-component-ComponentLaserHokuyoURGServer.sh pre-start
	sudo systemctl stop laserd3_serial.service
...
post-start)
	# first, trigger hooks as delivered from component ...
	bash startstop-hooks-component-ComponentLaserHokuyoURGServer.sh post-start
...
pre-stop)
	# first, trigger hooks as delivered from component ...
	bash startstop-hooks-component-ComponentLaserHokuyoURGServer.sh pre-stop
...
post-stop)
	# first, trigger hooks as delivered from component ...
	bash startstop-hooks-component-ComponentLaserHokuyoURGServer.sh post-stop
